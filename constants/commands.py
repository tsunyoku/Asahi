...
async def link_discord(user, args) -> str:
    ...
    await glob.db.execute(
        "UPDATE users SET discord = %s WHERE id = %s",
...
async def _clan_response(user: Player, args: list) -> Optional[str]:
    ...
    clan_name = args[1]
    clan = glob.clans.get(
        await glob.db.fetchval("SELECT id FROM clans WHERE name = %s", [clan_name]),
    )
...
async def clan_battle(user: Player, args: list) -> str:
    ...
    clan = glob.clans.get(
        await glob.db.fetchval("SELECT id FROM clans WHERE name = %s", [args[0]]),
    )
    ...
    if args[0] == "map":
        ...
        for mode in osuModes:
            ...
            for sc in scores_db:
                ...
                await glob.db.execute(
                    f"UPDATE {mode.table} SET pp = %s WHERE id = %s",
                    [score.pp, score.id],
                )
        ...
        maps = await glob.db.fetch(
            f"SELECT md5 FROM maps WHERE status >= {mapStatuses.Ranked}",
        )
...
async def mp_start(_, args: list, match: Match) -> str:
    ...
    if not args[0]:  # start now
        ...
        match.alert_tasks = [
            ...
            for countdown in (30, 15, 5, 4, 3, 2, 1)
            if countdown < timer
        ]
