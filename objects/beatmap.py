...
class Beatmap:
    ...
    async def np_msg(self, user) -> str:
        pp = {acc: await self.calc_acc(acc) for acc in (95, 97, 99, 100)}

        msg = (
            f"{self.embed}  // 95%: {pp[95]}pp | 97%: {pp[97]}pp | 99%: {pp[99]}pp | 100%: {pp[100]}pp"
    ...
    async def check_status(self):
            ...
            if resp.status != 200 or not resp:
                ...
                for table in ("scores", "scores_rx", "scores_ap"):
                    await glob.db.execute(
                        f"DELETE FROM {table} WHERE md5 = %s",
                        [self.md5],
                    )
        ...
        bmap = await glob.db.fetchrow(
            "SELECT id, status, frozen, `update` FROM maps WHERE id = %s",
            [self.id],
        )
        ...
        for m in data:
            ...
            if mid in in_db:
                ...
                if current != api and m["file_md5"] == self.md5:
                    ...
                    await glob.db.execute(
                        "UPDATE maps SET status = %s, nc = %s WHERE md5 = %s",
                        [self.status, self.nc, self.md5],
                    )
