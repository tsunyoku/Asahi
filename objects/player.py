...
from .match import slotStatus
from .match import Teams
...
class Player:
    ...
    async def login(cls, user: dict) -> "Player":
        ...
        self.friends = []
        async for user in glob.db.iter(
        ...
        db_achs = await glob.db.fetch(
            "SELECT ach FROM user_achievements WHERE uid = %s",
            [self.id],
        )
        for db in db_achs:
    ...
    @classmethod
    async def from_sql(cls, spc: Union[str, int], discord=False) -> Optional["Player"]:
        if discord:
            typ = "discord"
    ...
    async def set_stats(self) -> None:
        for mode in osuModes:
            ...
            if (
                not self.restricted
            ):  # might do my own aioredis wrapper just to solve this ugly
                ...
                cr = await glob.redis.zrevrank(
                    f"asahi:leaderboard:{mode.name}:{self.country_iso}",
                    self.id,
                )
    ...
    async def unfreeze(self, reason: str, fr: "Player") -> None:
        ...
        await glob.db.execute(
            "UPDATE users SET freeze_timer = 0 WHERE id = %s",
            [self.id],
        )
    ...
    async def unlock_ach(self, ach: "Achievement") -> None:
        await glob.db.execute(
            "INSERT INTO user_achievements (uid, ach) VALUES (%s, %s)",
            [self.id, ach.id],
        )
        self.achievements.append(ach)
