...
def running_via_asgi() -> bool:
    return any(
        map(sys.argv[0].endswith, ("hypercorn", "uvicorn",))
    )
...
def ensure_services() -> int:
    if glob.config.sql["host"] in ("localhost", "127.0.0.1",):
        for service in ("mysqld", "mariadb",):
            if os.path.exists(f"/var/run/{service}/{service}.pid"): break
        else:
    ...
    if (
        glob.config.redis["host"] in ("localhost", "127.0.0.1",) and 
        not os.path.exists("/var/run/redis/redis-server.pid")
    ):
