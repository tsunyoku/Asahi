...
class asahiBot(commands.Cog):
    ...
    async def link(self, ctx) -> None:
        if user := await glob.players.get(discord=ctx.author.id, sql=True):
            return await ctx.send(
                f"You already have your Discord account linked to a user called {user.name}!",
            )
        ...
        try:
            await ctx.send(
                "Linking initiated! Please check your DMs for further instructions.",
            )
            return await ctx.author.send(
                f"Please enter this command to the ingame bot ({glob.bot.name}) in order to link your account:\n```{glob.config.prefix}link {code}```",
            )
        except Exception:
            return await ctx.send(
                "There was an error when trying to DM you! Please ensure I am able to DM you and try again.",
            )
    ...
    async def profile(
        self,
        ctx,
    ) -> None:  # the way this is handled sucks ass and i'll clean this up later
        ...
        embed = Embed(title="")
        embed.set_author(
            name=f"{mode!r} profile for {user.full_name} - {stats.pp}pp (#{stats.rank} // {user.country_iso.upper()}#{stats.country_rank})",
