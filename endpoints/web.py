...
async def osuSearch(request: Request) -> Union[tuple, bytes]:
    ...
    async with glob.web.get(
        f"{glob.config.map_api}/api/search",
        params=request_args,
    ) as resp:
        if resp.status != 200:
    ...
    for _map in result:
        sorted_diffs = sorted(
            _map["ChildrenBeatmaps"],
            key=lambda sr: sr["DifficultyRating"],
        )
...
async def osuUpdates(request: Request) -> bytes:
    ...
    async with glob.web.get(
        "https://old.ppy.sh/web/check-updates.php",
        params=args,
    ) as resp:
        if resp.status != 200:
...
async def getMapScores(request: Request) -> bytes:
    ...
    if not bmap:
        ...
        if exists:
            return (
                b"1|false"  # bmap submitted but not up to date, send update available
            )
        else:
            glob.cache["unsub"].append(md5)
...
async def scoreSubmit(request: Request) -> bytes:
    ...
    if (
        cap is not None
        and s.pp >= cap
        and s.map.status & mapStatuses.GIVE_PP
        and glob.config.anticheat
        and not s.user.restricted
        and not s.user.priv & Privileges.Whitelisted
    ):  # ugly
    ...
    achievements = ""
    if (
        s.map.status & mapStatuses.GIVE_PP and not s.user.restricted
    ):  # TODO: hush-hush etc. achievements
        achs = []
...
async def lastFM(request: Request) -> bytes:
    ...
    if (
        b[0] != "a"
        or not glob.config.anticheat
        or player.priv & Privileges.BypassAnticheat
    ):
        return b"-3"
    ...
    if flags & osuFlags.SpeedHackDetected:  # speed hack
        return await player.restrict(
            reason="osu!anticheat flags (speed hack)",
            fr=glob.bot,
        )
    if flags & osuFlags.FL_CHEAT:  # fl tampering
        return await player.restrict(
            reason="osu!anticheat flags (fl cheating)",
            fr=glob.bot,
        )
    if flags & osuFlags.SpinnerHack:  # spin hack
        return await player.restrict(
            reason="osu!anticheat flags (spin hack)",
            fr=glob.bot,
        )
    if flags & osuFlags.FastPress:  # mania fast presses
        return await player.restrict(
            reason="osu!anticheat flags (mania fast presses)",
            fr=glob.bot,
        )
...
async def osuAddSetFavourite(request: Request) -> bytes:
    ...
    if await glob.db.fetchval(
        "SELECT 1 FROM favourites WHERE uid = %s AND sid = %s",
        [player.id, sid],
    ):
...
async def osuAddMapRating(request: Request) -> bytes:
    ...
    if "v" not in args:  # verifying we can rate the map?
        ...
        if not await glob.db.fetchval(
            "SELECT 1 FROM ratings WHERE md5 = %s AND uid = %s",
            [md5, player.id],
        ):
            return b"ok"
    ...
    avg = await glob.db.fetchval(
        "SELECT AVG(rating) AS average FROM ratings WHERE md5 = %s",
        [md5],
    )
