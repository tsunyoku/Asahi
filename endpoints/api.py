...
async def user(request: Request) -> Union[tuple, dict]:
    ...
    info = {
        ...
        "priv": user.priv,
        "clan": {"id": user.clan.id, "name": user.clan.name, "tag": user.clan.tag}
        if user.clan
        else None,
    }
...
async def playerStatus(request: Request) -> Union[tuple, dict]:
    ...
    if not id:
        return (
            400,
            {
                "message": "user could not be found! please check the username/id you specified and try again",
            },
        )
    ...
    if not search:
        for rank, uid in enumerate(lb):
            info = await glob.db.fetchrow(
                ...
                "LEFT OUTER JOIN stats ON stats.id = users.id WHERE users.id = %s".format(
                    lb_mode.name,
                ),
                [uid],
            )
            ...
            db_grades = await glob.db.fetchrow(
                ...
                'SUM(grade = "A") AS a FROM {0} WHERE uid = %s AND mode = %s'.format(
                    lb_mode.table,
                ),
                [uid, mode],
            )
...
async def getReplay(request: Request) -> Union[tuple, bytes]:
    ...
    if not file.exists():
        return (
            400,
            {
                "message": "replay couldn't be found. please check the score id and try again!",
            },
        )
...
async def playerScores(req: Request) -> Union[tuple, list]:
    ...
    query = (
        "SELECT t.id, t.md5, score, pp, acc, combo, mods, readable_mods, "
...
async def searchPlayers(req: Request) -> tuple[dict[str, object]]:
    ...
    users = await glob.db.fetch(
        "SELECT id, name FROM users WHERE safe_name LIKE %s",
        [f"{query}%"],
    )
    return users or ()
...
async def getClan(req: Request) -> Union[tuple, dict]:
    ...
    resp = {
        "info": {"id": clan.id, "name": clan.name, "tag": clan.tag},
        "owner": {
            "id": o.id,
            "name": o.name,
        ...
        "stats": {
            "score": await glob.db.fetchval(
                "SELECT score FROM clans WHERE id = %s",
                [clan.id],
            ),
            "rank": clan.rank,
